...
| Engineer (dev) | `Read`, `Write`, `Edit`, `Bash(git add:*)`, `Bash(git commit:*)` | `{status, files_modified}` |
...
| カスタム dev ロール (例: `front`, `back`) | Plan のタスクに基づいてコードを実装 | Read, Write, Edit, git add/commit のみ |
...
Worker の起動時、以下の3フェーズが順に実行される:

1. **Dev Phase** — リポジトリごとに Engineer Agent を並列実行。各エージェントがタスクを実装しコミット
2. **Review Loop** — リポジトリごとに Reviewer Agent が差分をレビュー。`request_changes` の場合は Engineer に差分とコメントをフィードバックし再実行 (最大3サイクル)
3. **Push & PR** — `gh` CLI で Draft PR を作成

### Git 差分管理

レビュー時の差分には2つのベースが使い分けられる:

- **`reviewBase`** (`git merge-base origin/<baseBranch> HEAD`) — Reviewer が見る差分。PR に含まれる全変更
- **`phaseBase`** (`git rev-parse HEAD` at phase start) — Engineer フィードバック用。このオーケストレーション実行で変更された部分のみ
...
- **リトライ**: hooks が失敗した場合、失敗出力を Engineer にフィードバックして修正を依頼（最大 3 回）
- **全リトライ失敗時**: そのリポジトリはスキップされ、エラーイベントが記録される
...
保存済みリポジトリ (`data/repositories.yaml`) にも `hooks` を設定でき、Item 作成時に引き継がれる。
